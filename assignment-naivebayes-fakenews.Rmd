...
```{r}
fake <- cleanDF %>% filter(label == "Fake")
real <- cleanDF %>% filter(label == "Real")
...
As you can check (how?) the DTM has almost 56528 columns and 15601 rows. Remember that each feature in the DTM is a word. Some words will have very few counts and therefore will have limited predictive power. To save on computation time we will eliminate words with low frequencies.
...
```{r}
nbayesModel <-  naiveBayes(as.matrix(trainDTM, trainDF$label, laplace = 1))
```
